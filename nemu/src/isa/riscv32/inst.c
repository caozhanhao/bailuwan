...
int isa_decode_branch(word_t pc, word_t inst, word_t *target_addr, bool *taken) {
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B, *taken = (src1 == src2); *target_addr = s->pc + imm;);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B, *taken = (src1 != src2); *target_addr = s->pc + imm;);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge, B, *taken = ((sword_t)src1 >= (sword_t)src2);
          *target_addr = s->pc + imm;);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu, B, *taken = (src1 < src2); *target_addr = s->pc + imm;);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu, B, *taken = (src1 >= src2); *target_addr = s->pc + imm;);
