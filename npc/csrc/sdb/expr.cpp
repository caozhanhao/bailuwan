...
{
    tokens = static_cast<Token*>(malloc(token_buf_size * sizeof(Token)));
    ...
    while (e[position] != '\0')
    {
        ...
        for (i = 0; i < NR_REGEX; i++)
        {
                ...
                pmatch.rm_so == 0)
            {
                ...
                {
                    token_buf_size *= 2;
                    auto* new_tokens = static_cast<Token*>(realloc(tokens, token_buf_size * sizeof(Token)));
                    Assert(new_tokens != nullptr, "realloc failed, new size: %lu",
                           token_buf_size);
                    tokens = new_tokens;
                }
                ...
                case TK_NUM:
                case TK_REG:
                    tokens[nr_token].str = static_cast<char*>(malloc(substr_len + 1));
                    strncpy(tokens[nr_token].str, substr_start, substr_len);
                    tokens[nr_token].str[substr_len] = '\0';
                    break;
                default:
                    tokens[nr_token].str = nullptr;
                    break;
                }
...
{
    Assert(e != nullptr && success != nullptr, "Bad arguments.");
...
{
    Assert(e != nullptr, "Bad arguments.");
